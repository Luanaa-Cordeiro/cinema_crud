...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <table class=" w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-800">
        ...
        @foreach($filmes as $filme)
            <tr class="bg-white white:bg-white-800 white:border-white-700 hover:bg-white-50">
                <td class="px-6 py-4">
                {{$filme->id}}
                </td>
                <td class="px-6 py-4">
                ...
                <a href="{{route('filmes.show',['filme' => $filme->id])}}"><button id="deletar">Deletar</button></a>
                </td>
            </tr>
            @endforeach
        </tbody>
