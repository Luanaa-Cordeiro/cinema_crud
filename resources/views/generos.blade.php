...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <table class=" w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-800">
        ...
        @foreach($generos as $genero)
            <tr class="bg-white white:bg-white-800 white:border-white-700">
                <td class="px-6 py-4">
                {{$genero->id}}
                </td>
                <td class="px-6 py-4">
                ...
                <a href="{{ route('generos.edit', ['genero' => $genero->id]) }}"><button id="editar">Editar</button></a>
                <a href="{{route('generos.show',['genero' => $genero->id])}}"><button id="deletar">Deletar</button></a>
                </td>
            </tr>
            @endforeach
        </tbody>
